...
public class GpsDetecting extends Service implements LocationListener {
    ...
    boolean isNetworkEnable         = false; // 네트워크 사용 여부
    ...
    final float MIN_DISTANCE_UPDTAE = 10.0f;         // GPS를 갱신하는 최소 이동 거리
    final long MIN_TIME_UPDTAE = 1000 * 6 * 1;      // GPS를 갱신하는 최소 시간
        ...
        try {
            if(isGPSEnable){
                if( locationManager!=null){
                    ...
                    // 위치 정보 획득 루트 1
                    // 단말기가 gps공급자를 통해 마지막으로 측정되었던 gps값을 획득
